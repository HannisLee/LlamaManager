...
| `_build_custom_command(service, host, port)` | 构建自定义服务启动命令并统一 host/port |
...
2. 模型下拉使用 `custom:<service_id>` 标识自定义服务，并显示 `[VLLM] 服务名`
3. 启动时后端使用 `shlex.split` 解析命令，不使用 shell，不要求用户写 `> log 2>&1 &`
4. 后端统一补齐或替换 `--host 0.0.0.0` 与 `--port <port>`
...
2. **启动区** — 模型/自定义服务下拉、Port、Extra Args、auto_kill、增量启动、GPU 选择、Start 按钮、自定义服务注册表单和已注册服务编辑/删除列表
