...
8. 「启动/运行中」列表按 service_id 匹配受管进程：未运行显示「启动」，运行中显示 PID 并提供 停止/重启/打开
...
2. **服务管理区** — 类型切换（llama.cpp / vLLM）的注册表单、已注册服务列表（编辑/删除）、启动/运行中列表（启动/停止/重启/打开）；auto_kill 与增量启动全局开关
...
| `renderRegisteredServices()` | 渲染「已注册服务」列表（编辑/删除） |
| `renderLaunchServices()` | 渲染「启动/运行中」列表，按 service_id 匹配运行状态切换按钮 |
