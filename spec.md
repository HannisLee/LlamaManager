...
| `_load_model_params()` | 从 settings.json 读取所有模型的启动参数 |
| `_save_model_param(model, port, extra_args)` | 保存单个模型的启动参数到 settings.json |
| `_load_custom_services()` | 从 settings.json 读取自定义服务注册表 |
...
9. 写入 `_managed_processes[pid]` 和 `settings.json.managed_processes`，记录 `process_create_time` 用于重启后校验 PID 身份
10. 按模型保存启动参数到 `settings.json.model_params`

**启动 vLLM / 自定义服务：**
1. vLLM 服务区通过 `/api/model-repositories` 扫描 `model_dir` 下全量下载的仓库目录，如 `Qwen--Qwen3.5`
2. 前端注册服务命令到 `settings.json.custom_services`，如 `python -m vllm.entrypoints.openai.api_server --model /home/lihan/models/Qwen--Qwen3.5 --host 0.0.0.0 --port 8085`
3. 已注册服务使用 `custom:<service_id>` 标识，并在 vLLM 服务区列表中直接启动
4. 启动时后端使用 `shlex.split` 解析命令，不使用 shell，不要求用户写 `> log 2>&1 &`
5. 自定义服务命令必须显式包含 `--port` 或 `--port=<port>`，后端只解析端口用于端口冲突检测、日志命名和代理记录，不再覆盖命令
6. stdout/stderr 重定向到 `logs/services/<type>-<name>-<port>-<timestamp>.log`
7. 自定义服务同样写入 `_managed_processes`，可在进程表中 Open / Stop / Restart
8. 已注册服务可在 vLLM 服务区直接编辑或删除，编辑时复用原 service_id 覆盖注册表记录
9. 点击启动后前端显示 PID / Port 并自动切换到新进程日志
...
2. **启动区** — GGUF 模型下拉、Port、Extra Args、auto_kill、增量启动、GPU 选择、Start 按钮
3. **vLLM 服务区** — 全量仓库目录下拉、服务名、启动命令、注册服务列表；已注册服务可启动、编辑、删除
4. **服务日志** — 进程日志下拉、Refresh 按钮、readonly textarea
5. **下载区** — HF 仓库ID、文件名（留空则全量下载整个仓库）、Download 按钮、强制重新下载复选框；可连续新增多个下载任务
6. **下载任务区** — 多任务进度列表、每任务 Cancel/Logs 操作、下载日志任务下拉、Refresh 按钮、readonly textarea
7. **设置区** — llama-server 路径、模型目录、GPU 历史小时数、Save/检测 llama.cpp/Rescan 按钮
...
| `loadModels()` | 扫描 GGUF 模型并填充启动区下拉框，刷新时保留当前已选模型 |
| `loadVllmRepositories()` | 扫描全量仓库目录并填充 vLLM 服务区下拉框 |
| `onVllmRepositoryChange()` | vLLM 仓库目录切换时显示路径并更新命令模板 |
| `loadCustomServices()` | 加载已注册 vLLM / 自定义服务列表 |
| `saveCustomService()` | 注册 vLLM / 自定义服务命令 |
| `editCustomService(id)` | 编辑已有 vLLM / 自定义服务 |
| `deleteCustomService(id)` | 删除已有 vLLM / 自定义服务 |
| `loadModelParams()` | 加载所有模型参数存入 `modelParams` |
| `onModelChange()` | GGUF 模型下拉切换时自动回填模型参数 |
| `loadGpuStatus()` | 获取 GPU 状态、波形历史和受管进程列表并更新 UI |
| `drawGpuWaveform()` | 绘制单张 GPU util 波形图 |
| `startServer()` | 收集表单参数调用 /api/start |
| `startCustomService(id)` | 从 vLLM 服务区启动已注册服务 |
...
| `model_params` | object | `{}` | 按模型路径保存的启动参数 |
| `custom_services` | object | `{}` | 用户注册的 vLLM / 自定义服务命令 |
| `managed_processes` | object | `{"processes":[]}` | LlamaManager 启动过的受管进程记录 |
| `gpu_history` | object | `{"samples":[]}` | GPU util 历史采样 |

### 内部状态结构

`settings.json.model_params` 按模型路径独立存储启动参数：
...
`settings.json.custom_services` 保存用户注册的非 llama.cpp 服务命令：
