...
- 标准 LLM 服务新增按 GPU 编号的固定代理地址：GPU 0 使用 `/0/`，GPU 1 使用 `/1/`
- 服务管理和 GPU 进程表的打开按钮改为打开对应 GPU 地址；ASR 服务仍固定打开 `/asr`
- 固定 GPU 代理仅接受已选择唯一 GPU 的运行中标准 LLM 服务；未选择、未运行或同一卡存在多个服务时返回明确错误，避免代理到错误模型
- spec.md 同步更新 GPU 固定代理接口和打开行为
